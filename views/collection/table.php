...
$(document).ready(function() {
    ...
    var records_api_url_base = <?= json_encode(TypeHelper::getApiUrl($this->type)) ?>;

    data_table_config = {
        dom: '<"top"lif>rt<"bottom"ip><"clear">',
        serverSide: true,
        ajax: function(data, callback, settings){
            var records_api_url = records_api_url_base;
            var filter_fields = [];
            ...
            for (let filter_field of filter_fields) {
                filter_agg_api_urls[filter_field] = records_api_url_base + '?limit=0&agg=' + encodeURIComponent(filter_field);
            ...
            if (data.search.value) {
                var v = data.search.value.split(/\s+/).map(function(v){ return '"' + v + '"'; }).join(' ');
    ...
    for (let key of url.searchParams.keys()) {
        if (key == 'page') {
            data_table_config.displayStart = (parseInt(url.searchParams.get(key)) - 1) * (data_table_config.pageLength || 10);
        } else if (key == 'limit') {
...
#filter-fields .card-body::-webkit-scrollbar { width: 5px; }
#filter-fields .card-body::-webkit-scrollbar-thumb { background-color: #ccc; }
</style>
<?php $this->yield_end() ?>
